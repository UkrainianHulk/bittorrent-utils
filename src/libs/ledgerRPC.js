...
const ledgerClient = new ledgerGrpc.Channels(
    grpcAdress,
...
const createAccount = ({ key }) =>
    new Promise((resolve, reject) => {
        ...
        const publicKeyUncompressed =
            keyObject instanceof PublicKey
                ? keyObject.uncompressed
                : keyObject.public.uncompressed
...
const transfer = ({ payerPrivateKey, recipientKey, amount }) =>
    new Promise((resolve, reject) => {
        ...
        const transferRequest = {
            ...
            recipient: {
                key:
                    recipientKeyObject instanceof PublicKey
                        ? recipientKeyObject.uncompressed
                        : recipientKeyObject.public.uncompressed,
            },
            amount,
        }
...
const createChannel = ({
    ...
    new Promise((resolve, reject) => {
        ...
        const channelCommit = {
            payer: {
                key: payerPrivateKeyObject.public.uncompressed.toString(
                    'base64'
                ),
            },
            recipient: {
                key:
                    recipientKeyObject instanceof PublicKey
                        ? recipientKeyObject.uncompressed.toString('base64')
                        : recipientKeyObject.public.uncompressed.toString(
                              'base64'
                          ),
            },
        ...
        const ChannelCommitMessageType = ledgerProto.lookupType(
            'ledger.ChannelCommit'
        )
        const ChannelCommitMessage =
            ChannelCommitMessageType.create(channelCommit)
        const serializedChannelCommitMessage =
            ChannelCommitMessageType.encode(ChannelCommitMessage).finish()
        const signature = payerPrivateKeyObject.hashAndSign(
            serializedChannelCommitMessage
        )
...
const closeChannel = ({
    ...
    new Promise((resolve, reject) => {
        ...
        const channelState = {
            ...
            to: {
                address: {
                    key: recipientPrivateKeyObject.public.uncompressed.toString(
                        'base64'
                    ),
                },
        ...
        const ChannelStateMessageType = ledgerProto.lookupType(
            'ledger.ChannelState'
        )
        const ChannelStateMessage = ChannelStateMessageType.create(channelState)
        const serializedChannelStateMessage =
            ChannelStateMessageType.encode(ChannelStateMessage).finish()
        const fromSignature = payerPrivateKeyObject.hashAndSign(
            serializedChannelStateMessage
        )
        const toSignature = recipientPrivateKeyObject.hashAndSign(
            serializedChannelStateMessage
        )
