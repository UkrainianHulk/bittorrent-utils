...
module.exports = class BitTorrent {
    constructor({ guiUrl, guiUsername, guiPassword }) {
        this.guiUrl = guiUrl
        ...
        const response = await fetch(url.href, {
            headers: {
                Authorization:
                    'Basic ' +
                    Buffer.from(
                        `${this.guiUsername}:${this.guiPassword}`
                    ).toString('base64'),
            },
        })
    ...
    async authorizedRequest(url) {
        try {
            ...
            if (response.status === 200) return response.json()
                ...
                log.warn(
                    `Response status ${response.status} - ` +
                        ...
                        ', did you restart client? Trying to relogin in 5 seconds...'
                )
            ...
            if (error.code === 'ECONNREFUSED') {
                log.warn(`${this.guiUrl} not responding, retry in 5 seconds...`)
                await new Promise((resolve) => setTimeout(resolve, 5000))
                return this.authorizedRequest(url)
            } else {
                throw error
